...
def predict_resp(x, ks, c, s=None, pi=None, pi_zero=None, zi=None, forcing=1.0) -> tuple[Any, Any]:
        ...
        if pi is None or len(pi) != n_components:
...
def _assign_basins(resp, data, ks, c, vect_t, preserve_mean_values, n_components, mean_forcing, final=False):
    ...
    # Use len(ks) as truth — infer_kinetics_temporal_scaled may return fewer
    # components than the original n_components if some basins are empty.
    K_actual = len(ks)
    basins: np.ndarray[Any, np.dtype[Any]] = np.zeros(n_cells, dtype=int)
    pi_final = {}
    pi = np.zeros(K_actual)
...
def compute_nu_star(
    ...
    if nu_init is None:
        ...
        if len(nu_init) != K:
            nu_init = np.ones(K) / K  # reinitialize if shape mismatch after merge
...
def _solve_mean_constraint(means_components, data_t, ks, c, nu_init, n_components, mean_forcing):
    ...
    """
    # Use actual number of components from means_components (may differ from n_components
    # after _refilter_merge reduces the component count)
    K = len(means_components)
    # Ensure nu_init matches K
    if nu_init is None or len(nu_init) != K:
        nu_init = np.ones(K) / K
    ...
    bounds: list[tuple[int, int]] = [(0, 1) for _ in range(K)]
