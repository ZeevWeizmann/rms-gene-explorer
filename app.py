...
# @st.fragment: all interactions (Load + gene select) stay inside fragment, no full app rerun
# st.form inside: file selection does not trigger rerun during transfer (avoids ClientDisconnect)
@st.fragment
def _upload_section():
    _has_data = st.session_state.get("_upload_umap") is not None
    with st.expander(T['genes_from_data'], expanded=_has_data):
